    ...
    <div class="is-flex flex-direction-row align-items-start">
      ...
      <div v-for="(item, i) of props.flowArr" :key="i" class="is-flex  flex-direction-row flow-item">
        <div class="is-flex flex-direction-column align-items-center">
          ...
          <span class="desc" :style="{ color: item.checkState === 1 ? '#11a816' : '#f4064d' }">{{ item.checkState === 1 ? t('已审核') : t('待审核') }}</span>
          <span v-if="item.checkResult === 1" class="desc" :style="{ color: '#11a816' }">{{ t('通过') }}</span>
          <span v-if="item.checkResult === 2" class="desc" :style="{ color: '#ef1d09' }">{{ t('拒绝') }}</span>
        </div>
        <div class="line"></div>
      </div>
      <div class="is-flex  flex-direction-row flow-item">
